...
@Component({
  selector: 'ft-file-drop',
  standalone: true,
  imports: [CommonModule],
  template: `
    <div #dragShieldRef class="drag-shield"></div>
...
export class FileDropComponent {
  ...
  constructor(dragShieldRef: ElementRef, zone: NgZone) {
  ...
  private getFileFromDropEvent(event: DragEvent): File | null {
    console.log(
      `🚀 ~ FileDropComponent ~ getFileFromDropEvent ~ event:`,
      event
    );
    ...
    if (event.dataTransfer?.items && event.dataTransfer.items.length) {
      for (var item of Array.from(event.dataTransfer.items)) {
        console.log(
          `🚀 ~ FileDropComponent ~ getFileFromDropEvent ~ item:`,
          item
        );
        if (item.kind === 'file') {
  ...
  private setupDragShieldEvents(): void {
    // NOTE: We are still outside of the Angular Zone.
    this.dragShieldRef.nativeElement.addEventListener(
      'dragover',
      this.handleShieldDragOver,
      false
    );
    this.dragShieldRef.nativeElement.addEventListener(
      'dragleave',
      this.handleShieldDragLeave,
      false
    );
    this.dragShieldRef.nativeElement.addEventListener(
      'drop',
      this.handleShieldDrop,
      false
    );
  }
  ...
  private teardownDragShieldEvents(): void {
    this.dragShieldRef.nativeElement.removeEventListener(
      'dragover',
      this.handleShieldDragOver,
      false
    );
